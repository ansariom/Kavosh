...
void Graph::setPath(const char *path) { 
    char file[256];
    sprintf(file, "adjMatrix.txt");
...
void Graph::outputFreq(int RAND, int subgraphCounter, const char *path) {
	FILE * cm;
	unsigned long i , j;

    char file[256];
//    sprintf(file, "%s/Frequencies.txt", path);
    sprintf(file, "Frequencies.txt");
...
void Graph::calculateZSCORE(int RAND, int subgraphCounter, const char *path) {
	FILE * cm;
	int i , j;
	printf("HI! Size = %llu\n", T->get_leafnum());
