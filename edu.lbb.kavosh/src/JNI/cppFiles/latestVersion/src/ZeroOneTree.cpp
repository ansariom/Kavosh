...
Leaf * tree::update_leaf(Leaf * l, int val) {
	if(l->count == ULLONG_MAX) {
		printf("Oops, Fatal error: There is a subgraph instant which occured more than %llu times, which does not fit in any datatype. We will extend Kavosh to support this case in future. Sorry!\n", ULLONG_MAX);
		exit(-1);
	}
	l->count += val;
	return l;
...
unsigned long long tree::get_leafnum() {
	return leaf_num;
}
...
unsigned long long * tree::extract() {
	C = new unsigned long long[get_leafnum()];
	head = 0;
	DFS(return_root());
	return C;
}
...
value tree::destroy() {
	register int i;
	v.C = new unsigned long long[get_leafnum()];
