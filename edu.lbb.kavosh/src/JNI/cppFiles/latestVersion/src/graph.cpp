...
void Graph::setPath2(char *path, int number) { 
    char file[256];
    sprintf(file, "%s/subGraph%d.txt", path, number);
    printf("Opening %s ...\n", file); 
    fflush(stdout);
    f2 = fopen(file, "w+");
    if(!f2) {
        printf("Can't open %s\n", path);
        sprintf(file, "result/subGraph%d.txt", path, number);
        printf("Writing to %s instead ...\n", file); 
    	f2 = fopen(file, "w+");
        if (!f2) {
            printf("Error again ... Sorry!\n");
...
void Graph::Classify(vertex **subgraph, int level) {
	...
	T->init_cur_node();
	if (!isRand) {
		int num = subgraphCounter / 100000;
		if (num > oldNum) {
			oldNum = num;
			fclose(f2);
			setPath2(output_directory, num);
		}
		for (int k = 0; k < subgraphSize; k++) {
			fprintf(f2, "%d ", tempSubgraph[k]);
		}
		fprintf(f2, "\n");
